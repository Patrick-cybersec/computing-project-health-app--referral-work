...
public class RecordingSprintActivity extends AppCompatActivity implements SensorEventListener {
    ...
    private void submitRecord() {
        ...
        String pacing = pacingValueText.getText().toString().replace(" s/m", "");
        ...
        if (rounds.isEmpty()) {
            Toast.makeText(this, "Please enter the number of rounds", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        if (!rounds.isEmpty()) {
            entries.add(new ExerciseFormatter.ExerciseEntry("Sprinting", "Rounds", rounds, "rounds"));
            hasValidEntry = true;
        }
        if (!distance.isEmpty() && !distance.equals("0")) {
            entries.add(new ExerciseFormatter.ExerciseEntry("Sprinting", "Distance", distance, "m"));
            hasValidEntry = true;
        }
        if (!pacing.isEmpty() && !pacing.equals("0")) {
            entries.add(new ExerciseFormatter.ExerciseEntry("Sprinting", "Pacing", pacing, "s/m"));
            hasValidEntry = true;
        }

        if (!hasValidEntry) {
            Toast.makeText(this, "Please enter at least one valid sprint metric (rounds, distance, or pacing)", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        try {
            ...
            Log.d("RecordingSprint", "Payload: " + payload.toString());
            ...
            client.newCall(request).enqueue(new Callback() {
                ...
                public void onFailure(Call call, IOException e) {
                    runOnUiThread(() -> Toast.makeText(RecordingSprintActivity.this,
                ...
                public void onResponse(Call call, Response response) throws IOException {
                    ...
                    if (response.isSuccessful()) {
                        runOnUiThread(() -> {
                            Toast.makeText(RecordingSprintActivity.this,
                                    "Record saved successfully!", Toast.LENGTH_SHORT).show();
                            Intent intent = new Intent(RecordingSprintActivity.this, Dashboard.class);
                            startActivity(intent);
                            finish();
                        });
                    } else {
                        Log.e("RecordingSprint", "Failed to save record: HTTP " + response.code() + ", Body: " + errorBody);
                        runOnUiThread(() -> Toast.makeText(RecordingSprintActivity.this,
